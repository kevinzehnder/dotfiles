function fs() {
	...
	local match=$(
		FZF_DEFAULT_COMMAND="$RG_BASE '$INITIAL_QUERY' 2>/dev/null || true" \
			fzf --ansi --disabled --query "$INITIAL_QUERY" \
			--bind "change:reload:$RG_BASE \$([ \$(cat $hidden_state_file) -eq 1 ] && echo '--hidden') {q} 2>/dev/null || true" \
			--bind "ctrl-f:unbind(change,ctrl-f)+change-prompt(2. fzf> )+enable-search+clear-query" \
			--bind "ctrl-h:execute-silent([ \$(cat $hidden_state_file) -eq 0 ] && echo 1 > $hidden_state_file || echo 0 > $hidden_state_file)+reload:$RG_BASE \$([ \$(cat $hidden_state_file) -eq 1 ] && echo '--hidden') {q} 2>/dev/null || true" \
