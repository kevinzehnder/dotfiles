function netsniff() {
	...
	local action=$(
		cat << EOF | fzf --ansi --header="Network Sniffing Tool" --prompt="Select action: "
Quick Presets
...
EOF
	...
	case "$action" in
		"Quick Presets")
			local preset=$(
				cat << EOF | fzf --ansi --header="Quick Presets" --prompt="Select preset: "
HTTP (port 80)
HTTPS (port 443)
...
EOF
		...
		"Custom Filter Builder")
			...
			# Add optional flags
			local flags=$(
				cat << EOF | fzf -m --ansi --header="Additional Options" --prompt="Select options (multi-select with Tab): "
